...
def denoise_branch(data):
    data = data.astype(np.float)
    regionlist = sorted(
        measure.regionprops(measure.label(data, connectivity=1)),
        key=lambda x: x.area,
        reverse=True,
    )
    for i in range(len(regionlist)):
        if regionlist[i].area < 300:
            data[
                regionlist[i].coords[:, 0],
                regionlist[i].coords[:, 1],
                regionlist[i].coords[:, 2],
...
def denoise_whole(whole):
    whole_mask = (whole > 0).astype(np.float)
    regionlist = sorted(
        measure.regionprops(measure.label(whole_mask, connectivity=1)),
        key=lambda x: x.area,
        reverse=True,
    )
    for i in range(1, len(regionlist)):
        if regionlist[i].area < 800:
            whole[
                regionlist[i].coords[:, 0],
                regionlist[i].coords[:, 1],
                regionlist[i].coords[:, 2],
...
def filling(AD, TFL):
    TFL_mask = (TFL > 0).astype(np.uint8)
    blank_z, blank_y, blank_x = np.where(AD - TFL_mask)
    TFL_reverse = np.logical_not(TFL_mask)
    dist, inds = distance_transform_edt(TFL_reverse, return_indices=True)  ### 距离变换比较耗时
    get_position = inds[:, blank_z, blank_y, blank_x].T
    TFL[blank_z, blank_y, blank_x] = TFL[
        get_position[:, 0], get_position[:, 1], get_position[:, 2]
    ]
    TL = (TFL == 1).astype(np.uint8)
    FL = (TFL == 2).astype(np.uint8)
    return TL, FL
                ...
                if len(measure.regionprops(label_img)) == 2:
                    ...
                    if (
                        ...
                        if seg_regions.max() == 2:
                            ...
                            break
    if not start:
        print("Start layer is not found, the middle layer is used")
        start = int((z.min() + z.max()) / 2)
...
def TFL_separation_edt2(
    Net1_AD, Net2_TL, Net2_FL, area_above_thr=(20, 50), area_below_thr=(20, 50)
):
    ############ 这个算法里面有几个参数要注意调节：###################################################################
    ############ 3.区域优化时的小区域上限和大区域下限，分别为上行(20,50)，下行（30,50）###############################
    ############ 3.为了避免把升主的区域弄进来，有一个距离阈值，默认设置为50 ##########################################
    ...
    for i in range(start, suplim + 1):
        if np.max(result_AD[i]) > 0:  ####### 确认当前层里面有前景主动脉
            ...
            if (
                ...
                ######## 需要确定之前的那一层是不是两个腔都有东西 ##########
                if lumen1[i - 1].max() > 0 and lumen2[i - 1].max() > 0:
                    for num in range(
                        1, seg_regions.max() + 1
                    ):  ### 目前我们直接采用遍历里面的所有的小区域，可能要优化
                        coord1_y, coord1_x = np.where(seg_regions == num)
                    ...
                    temp_slice = image.copy()
                    flap_area = get_flap_area(temp_slice)  ### 测试一下flap的面积
                    if flap_area < 20:  ### 如果flap很小，没有膜片，简单理解为这一层也本来只有一个区域
                        for num in range(1, seg_regions.max() + 1):  ### 提供了正常血管区段错分的鲁棒性
                            coord1_y, coord1_x = np.where(seg_regions == num)
                            score1 = containing_score(seg_regions == num, lumen1[i - 1])
                            score2 = containing_score(seg_regions == num, lumen2[i - 1])
                            if score1 > score2:
                        ...
                        if (
                            seg_regions.max() == 2
                        ):  ### 如果这个预测结果给出了两个区域，就把较大的放到里面，较小的放到另一个腔
                            if np.sum(lumen1[i + 1]) > 0:  ### 先确定是哪一个有
                                score1 = containing_score(
                                    lumen1[i - 1], seg_regions == 1
                                )
                                score2 = containing_score(
                                    lumen1[i - 1], seg_regions == 2
                                )
                                if score1 > score2:
                                    lumen1[i] = seg_regions == 1
                                ...
                                if score1 > score2:
                                    ...
                                    lumen2[i] = seg_regions == 2
                                    lumen1[i] = seg_regions == 1
                        else:  ### 如果这个预测结果给出了n多个区域
                            remain_slice = np.zeros(lumen1[i].shape)
                            for num in range(1, seg_regions.max() + 1):
                                coord1_y, coord1_x = np.where(seg_regions == num)
                                score1 = containing_score(
                                    seg_regions == num, lumen1[i - 1]
                                )
                                score2 = containing_score(
                                    seg_regions == num, lumen2[i - 1]
                                )
                                if score1 > score2:
                                    lumen1[i, coord1_y, coord1_x] = 1
                                elif score1 < score2:
                                    ...
                                    remain_slice[coord1_y, coord1_x] = 1
                            if np.sum(lumen1[i]) > 0:
                                lumen2[i] = remain_slice
                            else:
                                lumen1[i] = remain_slice
...
def resize_and_getTFL(
    ...
    result_256[result_256 > 0.5] = 1
    result_256[result_256 <= 0.5] = 0
...
def post_processing(Network_TL, Network_FL, result_AD):
    ...
    # lumen1, lumen2, suplim, start, inflim = TFL_separation_edt1(result_AD, tempTL, tempFL)
    lumen1, lumen2, suplim, start, inflim = TFL_separation_edt2(
        result_AD, tempTL, tempFL
    )
