...
# 原始压缩文件路径
SOURCE_DIR = "/yupeng/biz_rawdata/2c9180947481099601768364a16a23ad"
# 所有数据父级目录
ROOT_DIR = "/ssd/Jupiter/organ_seg/OrganSeg_ThiCK_DynamicReshape/"

# 解压缩后文件路径
UNZIP_DIR = "/ssd/Jupiter/organ_seg/OrganSeg_ThiCK_DynamicReshape/MVD5" 
# img和label文件路径
DATA_DIR = "/ssd/Jupiter/organ_seg/OrganSeg_ThiCK_DynamicReshape/mvd5_t2" # 原始npy
LIVER_DIR = "/ssd/Jupiter/organ_seg/OrganSeg_ThiCK_DynamicReshape/mvd5_t2_liver" # 归一化后liver
ORGAN_DIR = "/ssd/Jupiter/organ_seg/OrganSeg_ThiCK_DynamicReshape/mvd5_t2_organ" # 归一化liver + spleen
# txt路径
LIVER_TXT = "/ssd/Jupiter/organ_seg/organ_seg_splits/20201221_mvd5_liver.txt"
ORGAN_TXT = "/ssd/Jupiter/organ_seg/organ_seg_splits/20201221_mvd5_organ.txt"
...
def get_noramlize_organ(spacing_dict):
    ...
    with open(os.path.join(ROOT_DIR, 'spacing_mvd5.json')) as f_in:
        spacing_dict = json.load(f_in)
    ...
    for f in files:
        if 'DI' in f and '_new_liver_lab.npy' in f:
            ...
            if len(np.unique(mask)) == num_channel:
                print('error mask\tcase: {}'.foramt('DI_' + f.split('_')[1]))
...
if __name__ == '__main__':
    ...
    for f in tqdm(os.listdir(SOURCE_DIR)):
        if not os.path.exists(os.path.join(UNZIP_DIR, f.split('.')[0])):
            os.system('unzip {} -d {}'.format(os.path.join(ROOT_DIR, f), UNZIP_DIR))
    ...
    with open(os.path.join(ROOT_DIR, 'spacing_mvd5.json'), 'w') as f:
        json.dump(spacing_dict, f)
    

    with open(os.path.join(ROOT_DIR, 'spacing_mvd5.json'), 'r') as f:
