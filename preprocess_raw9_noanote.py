...
NORM_DIR = "/ssd/Jupiter/organ_seg/OrganSeg_ThiCK_DynamicReshape/mvd9_t2_norm" # 归一化后的所有数据

# txt路径
DATA_TXT = "/ssd/Jupiter/organ_seg/organ_seg_splits/20201222_mvd9.txt"
...
def do_thickness_norm(raw_img, raw_label, raw_thickness):
    ...
    new_slice_num = int(round(raw_thickness * raw_slice / Z_THICKNESS_NORM))
...
def get_noramlize_nomask(spacing_dict):
    """
    对于x,y进行缩放，z进行重采样
    ...
    for c in tqdm(caseids, total=len(caseids)):
        ...
        reshape_img = zoom(img.astype(np.float), zoom_sacle, order=3)
        # reshape_label = zoom(spleen_mask, zoom_sacle, order=0)
        reshape_label = np.zeros_like(reshape_img).astype(np.uint8)
        ...
        # print('Original shape {}\tNew shape {}\t{}'.format(img.shape, reshape_img.shape, count))
        np.save(os.path.join(NORM_DIR, '{}{}'.format(c, img_subfix)), reshape_img.astype(np.int16))
        # np.save(os.path.join(LIVER_DIR, '{}{}'.format(c, lab_subfix)), reshape_label.astype(np.uint8))
        # count += 1
...
if __name__ == '__main__':
    ...
    get_noramlize_nomask(spacing_dict)

    # write_txt(is_liver=False)
    # data_check(is_liver=False)
    '''
    '''